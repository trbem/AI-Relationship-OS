...
## v0.8.6 修复：联网导入 JSON 提取稳定性

- 增强 AI JSON 解析，支持 Markdown JSON、前后说明文字、平衡括号扫描和常见尾逗号修复。
- 免费网页导入在模型返回非法 JSON 时会自动尝试 JSON 修复，并在失败后使用更小批量来源重试。
- `EXTRACTION_FAILED` 会明确提示“找到网页，但模型没有返回可解析的人物 JSON”，不再误导为联网失败。

## v0.8.5 修复：联网导入运行中状态与搜索速度

- 修复任务仍在 `searching/extracting` 时前端误显示空预览的问题。
- 联网导入会显示搜索、抓取来源、AI 提取等阶段进度；超时后可继续等待、取消或稍后查看。
- 免费网页搜索优先抓取高质量 Top 10 来源，并过滤视频页、文库、知乎 403 等低质量/慢来源。
- MiMo/当前 AI 模型提取超时会显示可重试的结构化错误，不再伪装成“没有人物”。

## v0.8.4 优化：联网导入语言与稳定性

- 人物世界联网导入新增语言选择：默认中文，也支持自动识别和 English。
- 免费网页搜索按目标语言生成搜索词，中文模式会优先输出中文人物名、阵营、简介和关系。
- 单一消歧选项且已有联网验证人物时自动进入预览；多选项时显示已找到候选数量，避免误判为失败。
- 搜索到网页但 AI 未提取出人物时，会显示更明确的失败说明和来源失败摘要。
...
1. 运行 `dist/RelationshipOS-Setup-0.8.6.exe` 安装。
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts\Release-Windows.ps1 -Version 0.8.6
```
