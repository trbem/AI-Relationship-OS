...
ROLE_SANDBOX_NOTICE_ZH = "联网来源不可用，以下为模型基于通用知识生成的角色沙盘候选，未经 Web 来源验证。"
...
def _fallback_import_payload(query: str, limit: int, language: str) -> tuple[list[dict], list[dict]]:
    return _fallback_import_payload_v2(query, limit, language)
...
def _fallback_import_payload_v2(query: str, limit: int, language: str) -> tuple[list[dict], list[dict]]:
    if _is_three_kingdoms_topic(query):
        seeds = [
            ...
            ("关羽", "蜀汉", "重义气且威望极高的核心将领。", ["忠义", "骄傲"], ["守护兄弟情义", "维护蜀汉声望"]),
            ("张飞", "蜀汉", "勇猛直接、情绪外露的猛将。", ["勇猛", "急躁"], ["保护刘备集团", "证明武勇"]),
            ("诸葛亮", "蜀汉", "擅长谋划、治理和稳定政权的军师。", ["谨慎", "克制"], ["稳定政权", "完成托付"]),
            ("曹操", "曹魏", "善于用人与把握局势的枭雄。", ["果断", "多疑"], ["统一天下", "掌控主动权"]),
            ("孙权", "东吴", "善于平衡联盟、臣属和江东利益的统治者。", ["务实", "权衡"], ["守住江东", "扩张利益"]),
            ("周瑜", "东吴", "才华出众、维护江东利益的统帅。", ["敏锐", "骄傲"], ["击败强敌", "巩固江东"]),
            ("司马懿", "曹魏", "隐忍审慎、擅长长期布局的权臣。", ["隐忍", "审慎"], ["保存实力", "等待时机"]),
        ][:limit]
    else:
        seeds = [
            (
                f"{query} 角色 {index + 1}" if language == "zh" else f"{query} Character {index + 1}",
                "核心阵营" if language == "zh" else "Core faction",
                "模型生成的未验证角色沙盘候选。" if language == "zh" else "Model-generated unverified role-sandbox candidate.",
                [],
                [],
            )
        ...
        relationships.append(
            {
                ...
                "directed": True,
                "strength": 0.55,
                "description": "模型生成的未验证基础关系。" if language == "zh" else "Model-generated unverified relationship.",
                "confidence": 0.35,
                "source_type": "generated_unverified",
                "source_ref": None,
            }
...
def _is_three_kingdoms_topic(query: str) -> bool:
    ...
    return any(marker in normalized for marker in ["三国", "刘备", "曹操", "蜀汉", "three kingdoms"])
...
def _fallback_simulation_payload(
    ...
    return _fallback_simulation_payload_v2(
        people,
        relationships,
        scenario,
        rounds,
        ...
        output.append(
            {
                "round": round_number,
                "summary": (
                    f"第 {round_number} 轮：围绕“{scenario[:48]}”进行保守规则沙盘。"
                    if language == "zh"
                    else f"Round {round_number}: conservative sandbox around '{scenario[:48]}'."
                ),
...
def _zh_fallback_action(
    ...
    if round_number == 1:
        return f"先稳定{faction}内部预期，围绕“{scenario[:36]}”判断继承、士气和盟友反应。"
    if index % 3 == 0:
        return f"尝试把局势解释为有利于{faction}的机会，并以“{motive}”为目标争取主动。"
    if index % 3 == 1:
        return "保持观望并测试盟友立场，避免在信息不足时过早摊牌。"
    if relation_note:
        return f"利用既有关联关系施压或斡旋，优先降低{faction}的连锁风险。"
    return "调整资源和话语，等待下一轮信号更清晰后再行动。"
...
def _en_fallback_action(
    ...
    if round_number == 1:
        return f"Stabilizes expectations inside {faction} and reads ally responses around '{scenario[:36]}'."
    if index % 3 == 0:
        return f"Frames the disruption as an opportunity for {faction}, pursuing '{motive}'."
    if index % 3 == 1:
        return "Waits and tests ally positions while information is thin."
    if relation_note:
        return f"Uses existing relationships to pressure or mediate, reducing cascading risk for {faction}."
