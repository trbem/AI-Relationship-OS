...
def backup_data(
    ...
    payload = build_user_export(db, current_user.id)
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        archive.writestr(
            "relationship-os-backup.json",
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": 'attachment; filename="relationship-os-backup.zip"'},
    )
...
async def restore_data(
    ...
    if len(raw) > 200 * 1024 * 1024:
        raise HTTPException(status_code=413, detail="备份文件不能超过 200 MB")
    try:
        with zipfile.ZipFile(io.BytesIO(raw)) as archive:
            names = archive.namelist()
            if names != ["relationship-os-backup.json"]:
                raise ValueError("备份包内容无效")
            if archive.getinfo(names[0]).file_size > 500 * 1024 * 1024:
                raise ValueError("备份内容过大")
        ...
        raise HTTPException(status_code=400, detail=f"无效备份文件：{exc}") from exc
